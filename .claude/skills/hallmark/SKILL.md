...
**House brands (project-local token packs).** Before the catalog/custom fork, check the brief against `references/brands/`. If the brief names a brand that has a token pack there, load that file and use its locked tokens as the theme — skip the catalog rotation and the custom follow-up, but keep every other discipline (macrostructure variety, slop-test gates, pre-emit critique, honest copy). Currently packed: **Bandersnatch / the estate / the Historical Section** → [`references/brands/bandersnatch.md`](references/brands/bandersnatch.md). A house brand always wins over catalog; the user need not say "custom".
